export default function Page() {
    return (
        <main className="text-center p-12 relative text-black">
            <div className="flex justify-evenly gap-5">
                <div className="flex-grow min-w-[300px] p-2.5 rounded-lg">
                    <h1 className="text-5xl my-2 w-4/5 mx-auto">New Event</h1>
                    <p className="text-2xl my-4 w-3/5 mx-auto"></p>
                    <div className="flex justify-center">
                        <iframe width="500" height="500" src="https://www.runwaymobile.app"></iframe>
                    </div>
                </div>
                <div className="flex-grow min-w-[300px] p-2.5 rounded-lg">
                    <h1 className="text-5xl my-2 w-4/5 mx-auto">Sign up for membership!</h1>
                    <p className="text-2xl my-4 w-3/5 mx-auto"></p>
                    <div className="flex justify-center">
                        <iframe width="500" height="500" src="https://docs.google.com/forms/d/e/1FAIpQLSdN5HCIHy1OcgOwhx153N8IKlvmwQ4eH7fJLiPY-3H-6mGLLA/viewform"></iframe>
                    </div>
                </div>
            </div>
            <img src="data:image/png;base64," alt="Left Image" className="absolute left-0 top-1/2 transform -translate-y-1/2 w-[200px] h-auto" />
            <img src="data:image/png;base64," alt="Right Image" className="absolute right-0 top-1/2 transform -translate-y-1/2 w-[200px] h-auto" />
        </main>
    )
}